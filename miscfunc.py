...
def SetEmbed(title, description, url=False, colour=False, img=False, footer=False):
    ...
    if not url:
        ...
        mountEmbed = discord.Embed(
            title=title, description=description, color=colour, url=url
        )
    if not img:
        pass
    else:
        mountEmbed.set_image(url=img)
    if not footer:
        pass
    else:
        mountEmbed.set_footer(text=footer)
    return mountEmbed
...
async def thumbsdown(ctx):
    await ctx.message.add_reaction(
        "\N{THUMBS DOWN SIGN}"
    )  # https://stackoverflow.com/a/62856886/9654083
