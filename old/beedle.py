...
class beeedleCog(commands.Cog):
    ...
    def check(
        self, reaction, user
    ):  # https://stackoverflow.com/questions/63171531/how-do-you-check-if-a-specific-user-reacts-to-a-specific-message-discord-py
        return (
            user == self.BeedleCtx.author
            and str(reaction.emoji) in ["🔨"]
            and reaction.message == self.beetle
        )

    async def fail(self, context, scores):
        yay = ":D" if scores > 5 else ":)"
        feet = (
            f"But do collect {scores} brewcoin. {yay}"
            if scores > 0
            else "You didn't get any brewcoin either. Sucks to be you ig"
        )
        await context.send(
            embed=SetEmbed(
                title="You failed.",
                description="The beetle has already eaten you.\nGo to jail and do not collect $200.",
                footer=feet,
            )
        )
    ...
    async def beedle(self, context):
        ...
        while True:
            ...
            beetle = await context.send(
                f"Oh no, there's an annoying beetle! **Swat it in the next {times} seconds!**"
            )
            ...
            try:
                red = await self.bot.wait_for(
                    "reaction_add", check=self.check, timeout=times
                )
                if red is sus:
                    raise UnreachableCodeError(
                        "I THINK this code is unreachable...but I guess if you're seeing this I'm wrong."
                    )
            ...
            await ctx.send(
                f"You got it by {abs((int(laterTime - currentTime)) - times)} seconds! :beetle:"
            )
            if abs((laterTime - currentTime) - times) == 0:
                await ctx.send(
                    "In other words, you got it by the skin of your teeth! Nice job.\n"
        ...
        await context.send(
            embed=SetEmbed(
                title="YOU WIN!",
                description="The beetles realised that this wasn't helping them in the slightest. You Win :D",
                footer=f"Take {score} brewcoin kind stranger",
            )
        )
