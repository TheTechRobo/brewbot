...
class systemCog(commands.Cog):
    ...
    async def test(self, context):
        ...
        user = context.author
        await context.send(
            f"Hi {user}, you are senche raht :beer:\nAnd btw, I exist.\n\n\n\u200b"
        )
    ...
    async def spam(self, context, END):  # context is the content, end is the last thing
        ...
        if int(END) <= 16:
            ...
            role = discord.utils.find(
                lambda r: r.name == "Mini Brew", context.message.guild.roles
            )
            if not role in user.roles:
                await user.add_roles(role)
        else:
            await context.send(
                f"I don't want to block this, but it will probably really lag the server... So please limit auto spamming brew to 15..."
            )
    ...
    async def v(self, context):
        try:
            await context.send(
                embed=SetEmbed(
                    title="brewbot 0.2-wip",
                    footer="brewbot is closed source because of the rat TheRuntingMuumuu. Ping him a million times to get his attention! Or save up 1500 brewcoins and buy the perk! Whatever floats your boat!",
                    description="0.2 is in progress! It's about two thirds done!",
                )
            )
